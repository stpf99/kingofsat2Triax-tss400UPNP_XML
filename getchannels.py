	...
	if(newRadioChannel > 0):
		...
		# skip next 6 lines		
			...
			if("pid\">" in line):
				...
				url = "<channel number=" + '"' + "NR" + '"' + "><tuneType>DVB-S-AUTO</tuneType><visible>true</visible><type>radio</type><name>" + channelname + "</name><freq>" + frequency + "</freq><pol>" + polarity + "</pol><sr>" + symrate + "</sr><src>" + source + "</src><pids>" + apid + "</pids></channel>"
				for i in opidArray:
					url = url
		#		o.write(description + "\n")
				o.write(url + "\n")
		...
		if(newChannel < 6):
			...
			if("pid\">" in line):
				...
				# rtsp://@mmslave/?src=4&freq=10971&pol=h&msys=dvbs2&sr=29700&pids=0,501,502,507,503,504,505,502&x_pmt=501
				#description = "#EXTINF:0," + channelname
				url = "<channel number=" + '"' + "NR" + '"' + "><tuneType>DVB-S-AUTO</tuneType><visible>true</visible><type>tv</type><name>" + channelname + "</name><freq>" + frequency + "</freq><pol>" + polarity + "</pol><sr>" + symrate + "</sr><src>" + source + "</src><pids>" + vpid + "," + apid + "</pids></channel>"
				#url = "rtsp://@" + server + "/?src=" + source + "&freq=" + frequency + "&pol=" + polarity
